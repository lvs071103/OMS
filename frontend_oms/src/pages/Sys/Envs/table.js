...
export default function EnvTable () {
  ...
  const onSearch = async (q) => {
    const res = await http.get(`/v1/sys/config/env/list?q=${q}`)
    const { data } = res.data
    setEnv({
      list: data,
      count: data.length,
  ...
  const flushPage = async () => {
    const res = await http.get(`/v1/sys/config/env/list`, { params })
    const { data } = res
    // 刷新当前页面
  ...
  // 删除
  const delEnv = async (data, page) => {
    const resp = await http.delete(`/v1/sys/config/env/${data.id}`)
    if (resp.status === 200) {
        ...
        return (
          <Space size="middle">
            ...
            {isSuperuser ? <Popconfirm
              title="确认删除该条记录吗?"
              onConfirm={() => delEnv(data, params.page)}
              okText="确认"
              cancelText="取消"
            >
              <Button
